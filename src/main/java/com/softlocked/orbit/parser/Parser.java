    ...
    private static ASTNode parse(List<String> tokens, GlobalContext context, String clazzName) throws ParsingException {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            // 0. Importing a module
                ...
                body.addNode(new DeleteVarASTNode(next, next.hashCode()));
                            ...
                            for (Pair<String, Variable.Type> argument : arguments) {
                                constructorBody.addNode(
                                        new AssignVarASTNode(
                                                argument.first,
                                                argument.first.hashCode(),
                                                new VariableASTNode(("_" + argument.first), ("_" + argument.first).hashCode())
                                        )
                                );
                            ...
                            functions.put(
                                    ...
                                    new NativeFunction("cast", List.of(Variable.Type.STRING), Variable.Type.ANY) {
                                        ...
                                        public Object call(ILocalContext context, List<Object> args) {
                                            String type = (String) args.getFirst();
                                            ...
                                            if (type.equals("string")) {
                                                try {
                                                    ...
                                                    for (int i = 0; i < arguments.size(); i++) {
                                                        String name = arguments.get(i).first.substring(1);
                                                        Object value = new VariableASTNode(name, name.hashCode()).evaluate(context);
            ...
            if(token.matches(Utils.IDENTIFIER_REGEX) && !Utils.isKeyword(token)) {
                ...
                if (next != null && next.equals("[")) {
                    ...
                    if(next != null) {
                        VariableASTNode array = new VariableASTNode(token, token.hashCode());
                        ...
                        switch (next) {
                            case "++" -> {
                                value = new OperationASTNode(
                                        new VariableASTNode(token, token.hashCode()),
                                        new ValueASTNode(1),
                                        OperationType.ADD
                                );
                            }
...
//                        ));
                    ...
                    case 2 -> {
                        ...
                        if(initTokens.size() == 1) {
                            initNode = new DecVarASTNode(initTokens.getFirst(), initTokens.getFirst().hashCode(), new ValueASTNode(0), Variable.Type.LONG);
                        } else {
                            initNode = parse(initTokens, context);

                            if(initNode instanceof AssignVarASTNode assignVarASTNode) {
                                initNode = new DecVarASTNode(assignVarASTNode.variableName(), assignVarASTNode.hash(), assignVarASTNode.value(), Variable.Type.LONG);
                            }
                        }
                    ...
                    case 3 -> {
                        ...
                        ASTNode initNode;
                        if(initTokens.size() == 1) {
                            initNode = new DecVarASTNode(initTokens.getFirst(), initTokens.getFirst().hashCode(), new ValueASTNode(0), Variable.Type.INT);
                        } else {
                            initNode = parse(initTokens, context);

                            if(initNode instanceof AssignVarASTNode assignVarASTNode) {
                                initNode = new DecVarASTNode(assignVarASTNode.variableName(), assignVarASTNode.hash(), assignVarASTNode.value(), Variable.Type.INT);
                            }
                        }
                    ...
                    case 4 -> {
                        ...
                        if(initTokens.size() == 1) {
                            initNode = new DecVarASTNode(initTokens.getFirst(), initTokens.getFirst().hashCode(), new ValueASTNode(null), Variable.Type.ANY);
                        } else {
                            initNode = parse(initTokens, context);

                            if(initNode instanceof AssignVarASTNode assignVarASTNode) {
                                initNode = new DecVarASTNode(assignVarASTNode.variableName(), assignVarASTNode.hash(), assignVarASTNode.value(), Variable.Type.ANY);
                            }
                        }
    ...
    public static ASTNode postfixToAST(List<String> postfix, GlobalContext context) throws ParsingException, InterruptedException {
        ...
        for (int i = 0; i < postfix.size(); i++) {
            ...
            // If the token is - or +, but there is only 1 element on the stack, treat it as a prefix, and apply it to 0
                ...
                stack.push(new VariableASTNode(token, token.hashCode()));
            }
        }
